...
通过当前 skill 目录下的相对路径 `scripts/unity-rpc.py`（Python 单文件脚本）调用 Unity Editor 内的 Agent Bridge。

本文档中的脚本路径均以 skill 根目录 `.agents/skills/unity-rpc/` 为基准。
...
```bash
python3 scripts/unity-rpc.py call \

...
3. UnitySkills server can listen on `0.0.0.0:8090`, but the client must connect to the Unity host IP, for example `http://10.22.61.131:8090`

## Environment Variables

- `UNITY_SKILLS_HOST`: UnitySkills server host IP, for example `10.22.61.131`
- `UNITY_SKILLS_PORT`: reserved for external orchestration if needed; the helper currently uses the explicit `port` argument or default port `8090`
- `UNITY_SKILLS_ENDPOINT`: full endpoint such as `http://10.22.61.131:8090`, higher priority than host-based resolution
- `UNITY_RPC_HOST`: fallback host source reused when `UNITY_SKILLS_HOST` is not set

Examples:

```bash
export UNITY_SKILLS_HOST="10.22.61.131"
```

or:

```bash
export UNITY_SKILLS_ENDPOINT="http://10.22.61.131:8090"
```
...
```bash
# Health check
curl http://10.22.61.131:8090/health

# List all available skills
curl http://10.22.61.131:8090/skills

# Execute a skill
curl -X POST http://10.22.61.131:8090/skill/gameobject_create \

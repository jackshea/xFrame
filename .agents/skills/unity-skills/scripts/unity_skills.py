...
def _resolve_unity_host() -> str:
    """Resolve the UnitySkills server host from environment variables."""
    for env_key in ("UNITY_SKILLS_HOST", "UNITY_RPC_HOST"):
...
def _resolve_unity_url(port: int = DEFAULT_PORT) -> str:
    """Resolve the UnitySkills server URL from environment variables."""
    endpoint = os.environ.get("UNITY_SKILLS_ENDPOINT", "").strip()
    if endpoint:
        return endpoint

    return f"http://{_resolve_unity_host()}:{port}"
...
class UnitySkills:
    ...
    def __init__(self, port: int = None, target: str = None, url: str = None):
        ...
        Initialize client.
        Args:
            port: Connect to a specific UnitySkills port on the resolved host (e.g. 8091)
        ...
        if not self.url:
            if port:
                self.url = _resolve_unity_url(port)
            elif target:
                found_port = self._find_port_by_target(target)
                if found_port:
                    self.url = _resolve_unity_url(found_port)
                else:

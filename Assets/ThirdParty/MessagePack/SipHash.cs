...
namespace MessagePack
{
    ...
    internal class SipHash
    {
        ...
        public SipHash()
        {
            ...
            initialState0 = 0x736f6d6570736575UL ^ BinaryPrimitives.ReadUInt64LittleEndian(key);
            initialState1 = 0x646f72616e646f6dUL ^ BinaryPrimitives.ReadUInt64LittleEndian(key.Slice(sizeof(ulong)));
        ...
        public void GetKey(Span<byte> key)
        {
            ...
            BinaryPrimitives.WriteUInt64LittleEndian(key, initialState0 ^ 0x736f6d6570736575UL);
            BinaryPrimitives.WriteUInt64LittleEndian(key.Slice(sizeof(ulong)), initialState1 ^ 0x646f72616e646f6dUL);
        }
        ...
        public long Compute(ReadOnlySpan<byte> data)
        {
            unchecked
            {
                ...
                var finalBlockPosition = data.Length & ~7;

                // Process the input data in blocks of 64 bits
                for (var blockPosition = 0; blockPosition < finalBlockPosition; blockPosition += sizeof(ulong))
                {
                ...
                switch (data.Length & 7)
                {
                    case 7:
                        block |= MemoryMarshal.Read<uint>(data.Slice(finalBlockPosition)) |
                                 ((ulong)MemoryMarshal.Read<ushort>(data.Slice(finalBlockPosition + 4)) << 32) |
                                 ((ulong)data[finalBlockPosition + 6] << 48);
                        break;
                    case 6:
                        block |= MemoryMarshal.Read<uint>(data.Slice(finalBlockPosition)) |
                                 ((ulong)MemoryMarshal.Read<ushort>(data.Slice(finalBlockPosition + 4)) << 32);
                    ...
                    case 5:
                        block |= MemoryMarshal.Read<uint>(data.Slice(finalBlockPosition)) |
                                 ((ulong)data[finalBlockPosition + 4] << 32);
                        break;
                    case 4:
                        block |= MemoryMarshal.Read<uint>(data.Slice(finalBlockPosition));
                        break;
                    case 3:
                        block |= MemoryMarshal.Read<ushort>(data.Slice(finalBlockPosition)) |
                                 ((ulong)data[finalBlockPosition + 2] << 16);
                        break;
                    case 2:
                        block |= MemoryMarshal.Read<ushort>(data.Slice(finalBlockPosition));
                        break;

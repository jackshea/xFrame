...
namespace MessagePack
{
    ...
    public class MessagePackSerializerOptions
    {
        ...
        internal static readonly Regex AssemblyNameVersionSelectorRegex =
            new(@", Version=\d+.\d+.\d+.\d+, Culture=[\w-]+, PublicKeyToken=(?:null|[a-f0-9]{16})",
        ...
        public virtual void ThrowIfDeserializingTypeIsDisallowed(Type type)
        {
            if (type.FullName is string fullName && DisallowedTypes.Contains(fullName))
                throw new MessagePackSerializationException(
                    $"Deserialization attempted to create the type {fullName} which is not allowed.");
        }
        ...
        /// </summary>
        /// <param name="compressionMinLength">
        ///     The new value for the <see cref="CompressionMinLength" /> property. Must be a
        ///     positive integer.
        /// </param>
        /// <returns>The new instance; or the original if the value is unchanged.</returns>
        public MessagePackSerializerOptions WithCompressionMinLength(int compressionMinLength)
        {
            if (CompressionMinLength == compressionMinLength) return this;

            if (compressionMinLength <= 0) throw new ArgumentOutOfRangeException(nameof(compressionMinLength));
        ...
        /// </summary>
        /// <param name="suggestedContiguousMemorySize">
        ///     The new value for the <see cref="SuggestedContiguousMemorySize" />
        ///     property. Must be at least 256.
        /// </param>
        /// <returns>The new instance; or the original if the value is unchanged.</returns>
        public MessagePackSerializerOptions WithSuggestedContiguousMemorySize(int suggestedContiguousMemorySize)
        {
            if (SuggestedContiguousMemorySize == suggestedContiguousMemorySize) return this;

            if (suggestedContiguousMemorySize < 256)
                throw new ArgumentOutOfRangeException(nameof(suggestedContiguousMemorySize),

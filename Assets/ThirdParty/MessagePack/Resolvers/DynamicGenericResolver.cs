    ...
    internal static class DynamicGenericResolverGetFormatterHelper
    {
        ...
        internal static object? GetFormatter(Type t)
        {
            ...
            if (ti.IsGenericType)
            {
                ...
                // Tuple
                if (ti.FullName?.StartsWith("System.Tuple") is true)
                {
                    ...
                    switch (ti.GenericTypeArguments.Length)
                    {
                        ...
                        case 8:
                            ...
                            break;
                        default:
                            throw new MessagePackSerializationException("Unsupported arity for Tuple generic type: " +
                                                                        ti.Name);
                    }
                ...
                if (ti.FullName?.StartsWith("System.ValueTuple") is true)
                {
                    Type? tupleFormatterType = null;
                    switch (ti.GenericTypeArguments.Length)
                    {
                        ...
                        default:
                            throw new MessagePackSerializationException(
                                "Unsupported arity for ValueTuple generic type: " + ti.Name);
                    }
                ...
                {
                    if (ti.GenericTypeArguments[0] == typeof(byte)) return ByteArraySegmentFormatter.Instance;
                ...
                {
                    if (ti.GenericTypeArguments[0] == typeof(byte)) return ByteMemoryFormatter.Instance;
                ...
                {
                    if (ti.GenericTypeArguments[0] == typeof(byte)) return ByteReadOnlyMemoryFormatter.Instance;
                ...
                {
                    if (ti.GenericTypeArguments[0] == typeof(byte)) return ByteReadOnlySequenceFormatter.Instance;
            ...
            else
            {
                ...
                if (t == typeof(IEnumerable)) return NonGenericInterfaceEnumerableFormatter.Instance;

                if (t == typeof(ICollection)) return NonGenericInterfaceCollectionFormatter.Instance;

                if (t == typeof(IList)) return NonGenericInterfaceListFormatter.Instance;
            ...
            {
                // generic dictionary
                var dictionaryDef = ti.ImplementedInterfaces.FirstOrDefault(x =>
                    x.GetTypeInfo().IsConstructedGenericType() &&
                    x.GetGenericTypeDefinition() == typeof(IDictionary<,>));
                if (dictionaryDef != null &&
                    ti.DeclaredConstructors.Any(x => !x.IsStatic && x.GetParameters().Length == 0))
                {
                ...
                var dictionaryInterfaceDef = ti.ImplementedInterfaces.FirstOrDefault(x =>
                    x.GetTypeInfo().IsConstructedGenericType() &&
                ...
                if (dictionaryInterfaceDef != null)
                {
                    ...
                    foreach (var constructor in ti.DeclaredConstructors)
                    {
                        ...
                        if (parameters.Length == 1 &&
                            allowedParameterTypes.Any(allowedType =>
                                parameters[0].ParameterType.IsAssignableFrom(allowedType)))
                            return CreateInstance(typeof(GenericReadOnlyDictionaryFormatter<,,>),
                                new[] { keyType, valueType, t });
                    }
                }

                // generic collection
                var collectionDef = ti.ImplementedInterfaces.FirstOrDefault(x =>
                    x.GetTypeInfo().IsConstructedGenericType() &&
                ...
                foreach (var enumerableCollectionDef in ti.ImplementedInterfaces.Where(x =>
                             x.GetTypeInfo().IsConstructedGenericType() &&

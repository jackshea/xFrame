...
namespace xFrame.Runtime.Networking.AgentBridge
{
    ...
    public sealed class AgentRpcRouter
    {
        ...
        public AgentRpcRouter(AgentBridgeOptions options, AgentCommandRegistry registry,
            AgentReflectionInvoker reflectionInvoker = null, IXLogger logger = null)
        {
        ...
        public string Handle(string payload, string connectionId)
        {
            ...
            _logger.Debug(
                $"AgentBridge rpc direction=receive connectionId={normalizedConnectionId} payloadLength={payload?.Length ?? 0}");
            ...
            {
                _logger.Warning(
                    $"AgentBridge rpc direction=receive parse error connectionId={normalizedConnectionId} message={ex.Message}");
                return SerializeError(null, AgentRpcErrorCodes.ParseError, "Parse error.", ex.Message);
            }

            if (request == null || !string.Equals(request.JsonRpc, "2.0", StringComparison.Ordinal) ||
                string.IsNullOrWhiteSpace(request.Method))
            {
                _logger.Warning(
                    $"AgentBridge rpc direction=receive invalid request connectionId={normalizedConnectionId}");
            ...
            try
            {
                if (_registry.TryGet(request.Method, out var handler))
                {
                    ...
                    if (result.Error != null)
                        return SerializeErrorAndLog(request.Id, result.Error.Code, result.Error.Message,
                            normalizedConnectionId, isNotification, result.Error.Data);
                ...
                if (string.Equals(request.Method, "unity.reflect.invoke", StringComparison.Ordinal))
                {
                    ...
                    return SerializeResultAndLog(request.Id, reflectionResult.Result, normalizedConnectionId,
                        isNotification);
                }

                return SerializeErrorAndLog(request.Id, AgentRpcErrorCodes.MethodNotFound, "Method not found.",
                    normalizedConnectionId, isNotification);
            }
            catch (Exception ex)
            {
                _logger.Error(
                    $"AgentBridge rpc direction=error connectionId={normalizedConnectionId} method={request.Method}",
                    ex);
                return SerializeErrorAndLog(request.Id, AgentRpcErrorCodes.InternalError, "Internal error.",
                    normalizedConnectionId, isNotification, ex.Message);
            }
        }
        ...
        private string SerializeErrorAndLog(JToken id, int code, string message, string connectionId,
            bool isNotification, object data = null)
        {
            ...
            if (!isNotification)
            {
                ...
                _logger.Log(level,
                    $"AgentBridge rpc direction=send connectionId={connectionId} hasError=true code={code} message={message}");
            }

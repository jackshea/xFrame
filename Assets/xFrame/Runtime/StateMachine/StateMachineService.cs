...
namespace xFrame.Runtime.StateMachine
{
    ...
    public class StateMachineServiceService : IStateMachineService, ITickable, IDisposable
    {
        ...
        private readonly List<object> _updateableStateMachines = new();
        ...
        public StateMachine CreateStateMachine(string name, bool autoUpdate = true)
        {
            ...
            if (autoUpdate) _updateableStateMachines.Add(stateMachine);
        ...
        /// <param name="autoUpdate">是否自动更新</param>
        /// <returns>状态机实例</returns>
        public StateMachine<TContext> CreateStateMachine<TContext>(string name, TContext context,
        ...
        {
            if (_stateMachines.TryGetValue(name, out var stateMachine)) return stateMachine as StateMachine;
            return null;
        }
        ...
        {
            if (_stateMachines.TryGetValue(name, out var stateMachine)) return stateMachine as StateMachine<TContext>;
            return null;
        }
        ...
        private void Update()
        {
            for (var i = 0; i < _updateableStateMachines.Count; i++)
            {

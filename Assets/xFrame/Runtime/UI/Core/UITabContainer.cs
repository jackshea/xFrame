...
namespace xFrame.Runtime.UI
{
    ...
    public class UITabContainer : UIPanel
    {
        ...
        protected override void Awake()
        {
            ...
            // 如果没有指定页面容器，使用自身
            if (pageContainer == null) pageContainer = transform;
        }
        ...
        public int AddPage<T>(T page) where T : UITabPage
        {
            ...
            if (!_pagesByType.ContainsKey(pageType)) _pagesByType[pageType] = page;

            // 添加到名称映射
            _pagesByName[page.PageName] = page;

            // 初始化页面（如果还未创建）
            if (!page.IsCreated) page.InternalOnCreate();
        ...
        public void RemovePage(int index)
        {
            ...
            {
                var newIndex = index > 0 ? index - 1 : index < _pages.Count - 1 ? index + 1 : -1;
                if (newIndex >= 0)
                    SwitchPage(newIndex);
                else
                    CurrentPageIndex = -1;
            ...
            // 更新后续页面的索引
            for (var i = index; i < _pages.Count; i++) _pages[i].PageIndex = i;
        ...
        {
            if (index >= 0 && index < _pages.Count) return _pages[index];
            return null;
        }
        ...
        {
            if (_pagesByType.TryGetValue(typeof(T), out var page)) return page as T;
            return null;
        }
        ...
        {
            if (_pagesByName.TryGetValue(pageName, out var page)) return page;
            return null;
        }
        ...
        public void SwitchPage(int index, object data = null)
        {
            ...
            // 打开新页面（如果还未打开）
            if (!newPage.IsOpen) newPage.InternalOnOpen(data);
        ...
        public void NextPage()
        {
            ...
            var nextIndex = (CurrentPageIndex + 1) % _pages.Count;
        ...
        protected override void OnOpen(object data)
        {
            ...
            if (_pages.Count > 0)
            {
                var startIndex = Mathf.Clamp(defaultPageIndex, 0, _pages.Count - 1);
        ...
        protected override void OnShow()
        {
            ...
            // 显示当前页面
            if (CurrentPage != null) CurrentPage.InternalOnShow();
        }
        ...
        protected override void OnHide()
        {
            ...
            // 隐藏当前页面
            if (CurrentPage != null) CurrentPage.InternalOnHide();
        }
        ...
        protected override void OnUIDestroy()
        {
            ...
            foreach (var page in _pages)
            {
                ...
                if (page.IsCreated) page.InternalOnDestroy();
            }

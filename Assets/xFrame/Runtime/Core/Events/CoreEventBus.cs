...
namespace xFrame.Runtime.Core.Events
{
    ...
    public static class CoreEventBus
    {
        ...
        private static CoreGenericEventBus EventBus
        {
            get
            {
                if (_eventBus == null) _eventBus = new CoreGenericEventBus();
                return _eventBus;
            }
        }
        ...
        ///     订阅针对特定目标的事件
        /// </summary>
        public static void SubscribeToTarget<TCoreEvent>(object target, TargetedEventHandler<TCoreEvent> handler,
            float priority = 0)
            where TCoreEvent : ICoreEvent
        {
        ...
        ///     订阅来自特定源的事件
        /// </summary>
        public static void SubscribeToSource<TCoreEvent>(object source, TargetedEventHandler<TCoreEvent> handler,
            float priority = 0)
            where TCoreEvent : ICoreEvent
        {
    ...
    ///     目标事件处理器委托
    /// </summary>
    public delegate void TargetedEventHandler<in TCoreEvent>(TCoreEvent @event, object target, object source)
        where TCoreEvent : ICoreEvent;
    ...
    {
        // 日志输出器
        private static readonly TextWriter _errorWriter = Console.Error;
        ...
        public bool CurrentEventIsConsumed { get; private set; }

        public bool IsEventBeingRaised { get; private set; }
        ...
        {
            var previousIsRaising = IsEventBeingRaised;
            IsEventBeingRaised = true;
            CurrentEventIsConsumed = false;
            ...
            try
            {
                ...
                if (_listeners.TryGetValue(typeof(TCoreEvent), out var listeners))
                {
                    ...
                    {
                        if (CurrentEventIsConsumed) break;
                        try
                        {
                ...
                if (_targetedListeners.TryGetValue(typeof(TCoreEvent), out var targetedListeners))
                {
                    ...
                    foreach (var listener in sorted)
                    {
                        ...
                        try
                        {
                            ((TargetedEventHandler<TCoreEvent>)listener.Handler)(@event, listener.Target,
                                listener.Source);
                        }
                        catch (Exception ex)
                        {
            ...
            {
                IsEventBeingRaised = previousIsRaising;
                if (!previousIsRaising) CurrentEventIsConsumed = false;
            }
        }
        ...
        ///     立即触发事件（带目标和源）
        /// </summary>
        public bool RaiseImmediately<TCoreEvent>(ref TCoreEvent @event, object target, object source)
            where TCoreEvent : ICoreEvent
        {
            var previousIsRaising = IsEventBeingRaised;
            IsEventBeingRaised = false;
        ...
        ///     订阅事件
        /// </summary>
        public void Subscribe<TCoreEvent>(EventHandler<TCoreEvent> handler, float priority = 0)
            where TCoreEvent : ICoreEvent
        {
            var eventType = typeof(TCoreEvent);
            if (!_listeners.ContainsKey(eventType)) _listeners[eventType] = new List<Listener>();
        ...
        {
            var eventType = typeof(TCoreEvent);
            if (_listeners.TryGetValue(eventType, out var listeners)) listeners.RemoveAll(l => l.Handler == handler);
        }

        /// <summary>
        ///     订阅针对特定目标的事件
        /// </summary>
        public void SubscribeToTarget<TCoreEvent>(object target, TargetedEventHandler<TCoreEvent> handler,
            float priority = 0)
            where TCoreEvent : ICoreEvent
        {
            var eventType = typeof(TCoreEvent);
        ...
        ///     订阅来自特定源的事件
        /// </summary>
        public void SubscribeToSource<TCoreEvent>(object source, TargetedEventHandler<TCoreEvent> handler,
            float priority = 0)
            where TCoreEvent : ICoreEvent
        {
            var eventType = typeof(TCoreEvent);

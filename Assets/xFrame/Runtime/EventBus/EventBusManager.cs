...
namespace xFrame.Runtime.EventBus
{
    ...
    public class EventBusManager
    {
        ...
        private static readonly Lazy<EventBusManager> _instance = new(() => new EventBusManager());
        ...
        private void InitializeDefaultBuses()
        {
            ...
            // 创建UI事件总线（主线程安全）
            RegisterEventBus(UIBusName, EventBusFactory.CreateThreadSafe(5));

            // 创建游戏事件总线（标准配置）
            RegisterEventBus(GameBusName, EventBusFactory.CreateThreadSafe());

            // 创建网络事件总线（高并发）
            RegisterEventBus(NetworkBusName, EventBusFactory.CreateThreadSafe(20));
        }
        ...
        public bool UnregisterEventBus(string name)
        {
            ...
            if (_eventBuses.TryRemove(name, out var eventBus))
            {
                ...
                {
                    eventBus.Clear();
                    if (eventBus is IDisposable disposable) disposable.Dispose();
                }
                catch
                {
                    // 忽略异常
        ...
        public IEventBus GetOrCreateEventBus(string name, Func<IEventBus> factory = null)
        {
            ...
            return _eventBuses.GetOrAdd(name, _ => { return factory?.Invoke() ?? EventBusFactory.CreateThreadSafe(); });
        }
        ...
        /// <param name="eventData">事件数据</param>
        /// <param name="excludeBuses">排除的事件总线名称</param>
        public async Task BroadcastToAllAsync<T>(T eventData, params string[] excludeBuses) where T : IEvent
        {
            if (eventData == null)
                throw new ArgumentNullException(nameof(eventData));
    ...
    public static class EventBusManagerExtensions
    {
        ...
        /// <param name="busName">事件总线名称</param>
        /// <param name="eventData">事件数据</param>
        public static async Task PublishAsync<T>(this EventBusManager manager, string busName, T eventData)
            where T : IEvent
        {
            var eventBus = manager.GetEventBus(busName);
            if (eventBus != null) await eventBus.PublishAsync(eventData);
        }
        ...
        public static string Subscribe<T>(this EventBusManager manager, string busName, Action<T> handler,
            int priority = 0) where T : IEvent
        {
            var eventBus = manager.GetEventBus(busName);

...
namespace xFrame.Tests
{
    ...
    public class StateMachineServiceTests
    {
        ...
        public void CreateStateMachine_WithoutContext_ShouldCreateSuccessfully()
        {
            ...
            // Act
            var stateMachine = _service.CreateStateMachine(name, false);
        ...
        public void CreateStateMachine_WithContext_ShouldCreateSuccessfully()
        {
            ...
            // Act
            var stateMachine = _service.CreateStateMachine(name, context, false);
        ...
        public void CreateStateMachine_DuplicateName_ShouldThrowException()
        {
            ...
            _service.CreateStateMachine(name, false);

            // Act & Assert
            Assert.Throws<InvalidOperationException>(() => _service.CreateStateMachine(name, false),
        ...
        public void CreateStateMachine_WithAutoUpdate_ShouldAutoUpdate()
        {
            ...
            var stateMachine = _service.CreateStateMachine(name, context, true);
        ...
        public void GetStateMachine_WithoutContext_ShouldReturnCorrectInstance()
        {
            // Arrange
            const string name = "TestStateMachine";
            var createdStateMachine = _service.CreateStateMachine(name, false);
        ...
        public void GetStateMachine_WithContext_ShouldReturnCorrectInstance()
        {
            ...
            const string name = "TestStateMachineWithContext";
            var context = new TestContext();
            var createdStateMachine = _service.CreateStateMachine(name, context, false);
        ...
        public void GetStateMachine_WrongType_ShouldReturnNull()
        {
            // Arrange
            const string name = "TestStateMachine";
            _service.CreateStateMachine(name, false); // 创建不带上下文的状态机
        ...
        public void Tick_MultipleStateMachines_ShouldUpdateAll()
        {
            ...
            var sm1 = _service.CreateStateMachine("SM1", context1, true);
            var sm2 = _service.CreateStateMachine("SM2", context2, true);
        ...
        public void Tick_MixedUpdateModes_ShouldOnlyUpdateAutoUpdateStateMachines()
        {
            ...
            var sm1 = _service.CreateStateMachine("SM1", context1, true);
            var sm2 = _service.CreateStateMachine("SM2", context2, false);
        ...
        {
            // Arrange
            var sm = _service.CreateStateMachine("SM", true);
        ...
        {
            // Arrange
            _service.CreateStateMachine("SM1", false);
            _service.CreateStateMachine("SM2", false);
        ...
        public void Tick_AfterDispose_ShouldNotThrow()
        {
            // Arrange
            var context = new TestContext();
            var sm = _service.CreateStateMachine("SM", context, true);
            ...
            // Assert
            Assert.DoesNotThrow(() => ((ITickable)_service).Tick(),
        ...
        {
            // Act
            var sm = _service.CreateStateMachine("", false);
        ...
        public void CreateStateMachine_ManyStateMachines_ShouldHandleCorrectly()
        {
            // Arrange & Act
            const int count = 100;
            for (var i = 0; i < count; i++) _service.CreateStateMachine($"SM{i}", false);
        ...
        public void CreateAndRemove_Repeatedly_ShouldWorkCorrectly()
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                var sm = _service.CreateStateMachine(name, false);
        ...
        public void FullLifecycle_ShouldWorkCorrectly()
        {
            ...
            // Act - 创建状态机
            var sm = _service.CreateStateMachine(name, context);

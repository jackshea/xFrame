...
// 2. 使用批量操作减少IO次数
var items = new Dictionary<string, PlayerData>
{
    { "player1", player1Data },
    { "player2", player2Data },
    { "player3", player3Data }
};
await _persistence.SaveBatchAsync<PlayerData>(items);

// 3. 启用内存缓存减少重复读取
var config = new PersistenceConfig
{
    EnableCache = true,
    CacheExpirationSeconds = 300 // 5分钟过期
};
...
{
    EnableBackup = true,
    MaxBackupCount = 3  // 保留最近3个备份
};
```

启用备份后，每次保存数据时会自动创建备份：
- `key.backup.0` - 最新备份
- `key.backup.1` - 次新备份
- `key.backup.2` - 最旧备份
...
```csharp
// 从最新备份恢复
bool success = _persistence.RestoreFromBackup("player_data");

// 从指定备份恢复（索引0=最新，1=次新...）
bool success = _persistence.RestoreFromBackup("player_data", 1);

// 异步恢复
bool success = await _persistence.RestoreFromBackupAsync("player_data");
```
...
```csharp
// 获取可用备份数量
int backupCount = _persistence.GetBackupCount("player_data");

// 删除所有备份
_persistence.DeleteAllBackups("player_data");
```
...
// 同步批量保存
_persistence.SaveBatch<PlayerData>(items);
...
```csharp
var keys = new[] { "slot1", "slot2", "slot3" };

// 同步批量加载
var results = _persistence.LoadBatch<PlayerData>(keys);

// 异步批量加载
var results = await _persistence.LoadBatchAsync<PlayerData>(keys);

// 结果是字典，只包含存在的数据
foreach (var kvp in results)
{
    Debug.Log($"{kvp.Key}: {kvp.Value.playerName}");
}
```
...
{
    EnableCache = true,
    CacheExpirationSeconds = 300  // 缓存5分钟过期
};
```

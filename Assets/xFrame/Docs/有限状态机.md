...
### 4. IStateMachineModule / StateMachineModuleService
状态机模块接口和服务实现，支持创建和管理多个状态机实例。集成VContainer的ITickable接口实现自动更新。
...
stateMachine.Name = "PlayerStateMachine"; // 设置状态机名称

// 通过事件总线监听状态改变事件
xFrameEventBus.SubscribeTo<StateChangedEvent<PlayerContext>>((ref StateChangedEvent<PlayerContext> evt) =>
{
    Debug.Log($"状态改变: {evt.PreviousState?.GetType().Name} -> {evt.CurrentState?.GetType().Name}");
});
...
### 使用状态机模块（VContainer集成）

```csharp
using VContainer;
using xFrame.StateMachine;

// 状态机模块已在xFrameLifetimeScope中自动注册
// 通过依赖注入使用
public class GameController : MonoBehaviour
{
    ...
    // VContainer自动注入
    [Inject]
    public void Construct(IStateMachineModule stateMachineModule)
    {
        _stateMachineModule = stateMachineModule;
    }
    ...
    void Start()
    {
        ...
        var stateMachine = _stateMachineModule.CreateStateMachine("PlayerSM", context, autoUpdate: true);
...
| `GetStateMachine(name)` | 获取状态机（不带上下文） |
| `GetStateMachine<TContext>(name)` | 获取状态机（带上下文） |
| `RemoveStateMachine(name)` | 移除状态机 |

**注意**: StateMachineModuleService实现了VContainer的ITickable接口，会自动每帧更新所有autoUpdate为true的状态机，无需手动调用Update。

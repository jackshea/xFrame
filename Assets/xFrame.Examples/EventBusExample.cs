...
namespace xFrame.Examples
{
    ...
    public class EventBusExample : MonoBehaviour
    {
        ...
        private void OnDestroy()
        {
            ...
            if (_eventBus != null)
            {
                ...
                if (_eventBus is IDisposable disposable) disposable.Dispose();
            }
        ...
        private void BasicSubscribeAndPublishExample()
        {
            ...
            // 订阅玩家生命值变化事件
            _playerHealthSubscriptionId = _eventBus.Subscribe<PlayerHealthChangedEvent>(OnPlayerHealthChanged, 1);

            // 订阅游戏状态变化事件
            _gameStateSubscriptionId = _eventBus.Subscribe<GameStateChangedEvent>(OnGameStateChanged);

            // 使用处理器类订阅事件
            var playerDeathHandler = new PlayerDeathEventHandler();
            _eventBus.Subscribe(playerDeathHandler);
        ...
        private async void AsyncEventHandlingExample()
        {
            ...
            _eventBus.SubscribeAsync(asyncHandler);
        ...
        private void FilterAndInterceptorExample()
        {
            ...
            // 添加事件过滤器
            var healthFilter = new PlayerHealthFilter(20);
            _eventBus.AddFilter(healthFilter);

            // 添加事件拦截器
            var loggingInterceptor = new LoggingInterceptor<PlayerHealthChangedEvent>();
            _eventBus.AddInterceptor(loggingInterceptor);
        ...
        private void PerformanceMonitoringExample()
        {
            ...
            // 获取事件总线统计信息
            if (_eventBus is ThreadSafeEventBus threadSafeBus) Debug.Log($"事件总线统计: {threadSafeBus.GetStatistics()}");
    ...
    public class PlayerHealthFilter : IEventFilter<PlayerHealthChangedEvent>
    {
        ...
        {
            // 只处理生命值大于最小值的事件
            var shouldHandle = eventData.Data.NewHealth >= _minHealth;

            if (!shouldHandle) Debug.Log($"事件被过滤: 玩家生命值 {eventData.Data.NewHealth} 低于最小值 {_minHealth}");

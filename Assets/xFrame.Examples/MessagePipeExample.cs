...
namespace xFrame.Examples
{
    ...
    public class MessagePipeExample : MonoBehaviour, IStartable, IDisposable
    {
        ...
        // 依赖注入的服务
        [Inject] private IPublisher<GameEvent> gameEventPublisher;
        [Inject] private ISubscriber<GameEvent> gameEventSubscriber;
        [Inject] private IPublisher<string, HealthChangedEvent> healthEventPublisher;
        [Inject] private ISubscriber<string, HealthChangedEvent> healthEventSubscriber;
        [Inject] private IAsyncPublisher<GameEvent> asyncGameEventPublisher;
        [Inject] private IAsyncSubscriber<GameEvent> asyncGameEventSubscriber;
        ...
        public void RunEventTest()
        {
            ...
            var specialEvent = new GameEvent
            {
                ...
                value = UnityEngine.Random.Range(100, 200),
                timestamp = DateTime.Now
            };
            ...
            var asyncEvent = new GameEvent
            {
                ...
                value = UnityEngine.Random.Range(200, 300),
                timestamp = DateTime.Now
            };
            ...
            {
                currentHealth = UnityEngine.Random.Range(50, 100),
                maxHealth = 100,
                healthPercentage = UnityEngine.Random.Range(0.5f, 1f)
            };
            ...
            {
                currentHealth = UnityEngine.Random.Range(30, 80),
                maxHealth = 100,
                healthPercentage = UnityEngine.Random.Range(0.3f, 0.8f)
            };
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("MessagePipe示例控制", new GUIStyle(GUI.skin.label) { fontSize = 16, fontStyle = FontStyle.Bold });
            
            GUILayout.Space(10);
            
            if (GUILayout.Button("运行事件测试"))
            {
                RunEventTest();
            }
            
            if (GUILayout.Button("测试请求/响应"))
            {
                TestRequestResponse();
            }

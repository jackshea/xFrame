...
**Architecture:** Runtime 层提供协议与路由核心，Editor 层用 Fleck 承载传输通道与连接生命周期。默认白名单命令路由，开发模式可启用受限反射调用。通过仓库内 Skill 与 Node.js CLI 统一 OpenCode 调用入口。

**Tech Stack:** Unity 2021.3, C#, Fleck, VContainer, NUnit(EditMode), Node.js 22
...
### Task 6: 提供 OpenCode Agent Skill 与 Node.js 客户端

**Files:**
- Create: `skills/unity-rpc/SKILL.md`
- Create: `scripts/agent/unity-rpc.js`
- Create: `scripts/agent/unity-rpc.test.js`

**Step 1: Write failing Node.js tests**

- 覆盖参数解析、请求封装、错误响应处理。

**Step 2: Verify RED**

Run: `node --test scripts/agent/unity-rpc.test.js`  
Expected: FAIL
...
- `node --test scripts/agent/unity-rpc.test.js`
